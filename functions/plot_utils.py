...
def plot_df_response(df_response, up_col, down_col, point_col, ax=None, color='teal', alpha=0.4, label='95 % CI', fill_betwen=True, path_to_save=None, title='Age group'):
    if not ax:
        fig, ax = plt.subplots(1, 1, figsize=(12.5, 7))

    ax.plot(df_response.index.values, df_response[point_col], color=color)
    if fill_betwen:
        ax.fill_between(df_response.index.values, df_response[down_col], df_response[up_col], color=color, alpha=alpha, label=label)
    ax.xaxis.set_major_locator(mdates.MonthLocator())
    ax.xaxis.set_major_formatter(mdates.DateFormatter('%b'))
    ax.xaxis.set_minor_locator(mdates.DayLocator())
    ax.xaxis.set_major_locator(mdates.WeekdayLocator())
...
def plot_df_response_medians(df_response, up_col, down_col, point_col, ax=None, color='teal', label='95 % CI', fill_betwen=True, path_to_save=None, title='Age group'):
    if not ax:
        fig, ax = plt.subplots(1, 1, figsize=(12.5, 7))

    ax.plot(df_response.index.values, df_response[point_col], color=color, label=label)
    if fill_betwen:
        ax.fill_between(df_response.index.values, df_response[down_col], df_response[up_col], color=color, alpha=0.6, label=label)
    ax.xaxis.set_major_locator(mdates.MonthLocator())
    ax.xaxis.set_major_formatter(mdates.DateFormatter('%b'))
    ax.xaxis.set_minor_locator(mdates.DayLocator())
    ax.xaxis.set_major_locator(mdates.WeekdayLocator())
